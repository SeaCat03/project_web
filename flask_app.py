...
def lk():
    person_id = 328781630
    old_price = ''
    cart, cart_count = check_cart(db, user_table, person_id)
    fav, fav_count = check_fav(db, user_table, person_id)

    cart_pictures = ''
    for x in cart:
        data = get_info_about_offer(db, offer_table, x)
        active = 'active'

        for j in data['picture']:
            cart_pictures += f'''<h4 class="d-md-flex d-lg-flex d-xl-flex d-xxl-flex justify-content-md-center justify-content-lg-center justify-content-xl-center justify-content-xxl-center card-title {active}">Подушка от LongDog<br></h4><img src="{j}" alt="Slide Image" style="width: 100%;">'''
            active = ''
        cart_picture = ''
    return render_template('user.html', cart_pictures=cart_pictures, cart_count=cart_count, fav_count=fav_count)
...
def product(offer_id):
    ...
    if data['actually_cost'] != data['always_cost']:
        ...
        old_price = f'''<div class="col d-md-flex d-xxl-flex align-items-md-center align-items-xxl-center">
            <h1 class="display-6" style="font-size: 280%;"><span style="text-decoration: line-through;">{p}₽</span><br></h1>
            </div>'''
...
def bot():
    ...
    if data['type'] == 'message_new':
        ...
        if message['text'] == 'Начать':
            ...
            billId, Url = set_bill(db, message['peer_id'], offer.actually_cost, [offer.actually_cost])
            ...
            send_message(db, message['peer_id'], 'Секунду...', table=user_table)
            user = db.session.query(user_table).filter_by(person_id=int(message['peer_id'])).first()
            if user == None or len(user.cart) == 0:
                send_message(db, message['peer_id'], 'Пока что корзина пуста', table=user_table)
                ...
                send_message(db, message['peer_id'], 'Корзина: \n' + res, table=user_table)
            return 'ok'
            ...
            if status == 'WAITING':
                send_message(db, message['peer_id'], 'Пока что платеж не пришел. Подождите пока платежь пройдёт и повторите запрос', table=offer_table)
